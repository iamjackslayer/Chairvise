...
export default {
  ...
  methods: {
    ...
    showLinkDetails(event, link) {
      var info = [
        this.nodes[link.sid]._labelClass,
        this.nodes[link.tid]._labelClass,
        link.name
      ];
      var newMsg = "";
    ...
    updateVisualisation({ result, selections, extraData }) {
      ...
      this.partialResult.forEach((element, index) => {
        ...
        graphLinks[index] = {
          sid: distinctNodes
            .filter(x => x._labelClass == element[selections[0].rename])
            .map(x => x.id)[0],
          tid: distinctNodes
            .filter(x => x._labelClass == element[selections[1].rename])
            .map(x => x.id)[0],
      ...
      if (extraData.isColorfulBar) {
        distinctNodes.forEach((node, index) => {
          node._color = generateBackgroundColor(this.partialResult.length)[
            index % this.partialResult.length
          ];
        });
        graphLinks.forEach((link, index) => {
          link._color = generateBackgroundColor(this.partialResult.length)[
            index % this.partialResult.length
          ];
        });
      } else {
