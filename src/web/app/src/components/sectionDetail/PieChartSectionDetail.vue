...
export default {
  ...
  methods: {
    updateVisualisation({ result, extraData }) {
      ...
      let remainedResult = result.slice(
        extraData.numOfResultToDisplay,
        result.length
      );

      // process category
      this.labels = displayedResult.map(
        record => record[extraData.categoryFieldName]
      );
      ...
      // process value
      let data = displayedResult.map(
        record => record[extraData.valueFieldName]
      );
      if (remainedResult.length !== 0) {
        // take sum of the rest
        data.push(
          remainedResult
            .map(record => record[extraData.valueFieldName])
            .reduce((a, b) => a + b, 0)
        );
      }
