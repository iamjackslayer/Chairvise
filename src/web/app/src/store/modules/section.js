...
export default {
  ...
  actions: {
    ...
    async addSectionDetail(
      { commit },
      { presentationId, selectedNewSection, dataSet }
    ) {
      commit("setSectionListLoading", true);

      let newSection = PredefinedQueries[selectedNewSection].data;
      newSection = JSON.parse(
        JSON.stringify(newSection).replace(/\${PLACEHOLDER_DATA_SET}/g, dataSet)
      );

      await axios
        .post(`/api/presentations/${presentationId}/sections`, newSection)
        .then(response => {
    ...
    async saveSectionDetail(
      ...
      await axios
        .put(`/api/presentations/${presentationId}/sections/${id}`, {
          title,
          description,
          dataSet,
          selections,
    ...
    async deleteSectionDetail({ commit }, { id, presentationId }) {
      ...
      await axios
        .delete(`/api/presentations/${presentationId}/sections/${id}`)
        .then(() => {
    ...
    async sendPreviewAnalysisRequest(
      { commit },
      {
        presentationId,
        id,
      ...
      await axios
        .post(`/api/presentations/${presentationId}/analysis`, {
          dataSet,
          selections,
          involvedRecords,
          filters,
