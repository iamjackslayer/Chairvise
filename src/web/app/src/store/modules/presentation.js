...
export default {
  ...
  mutations: {
    ...
    deleteFromPresentationList(state, payload) {
      const index = state.presentationList.findIndex(
        presentation => presentation.id === payload
      );
  ...
  actions: {
    ...
    async savePresentation({ commit, state }) {
      commit("setPresentationFormLoading", true);
      await axios
        .post("/api/presentations", state.presentationForm)
        .then(response => {
    ...
    async updatePresentation({ commit, state }) {
      ...
      await axios
        .put(
          "/api/presentations/" + state.presentationForm.id,
          state.presentationForm
        )
        .then(response => {
    ...
    async deletePresentation({ commit }, payload) {
      commit("setPresentationFormLoading", true);
      await axios
        .delete("/api/presentations/" + payload)
        .then(() => {
