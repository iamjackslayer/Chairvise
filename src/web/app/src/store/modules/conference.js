...
export default {
  ...
  mutations: {
    ...
    deleteFromConferenceList(state, payload) {
      const index = state.conferenceList.findIndex(
        conference => conference.id === payload
      );
  ...
  actions: {
    ...
    async updateConference({ commit, state }) {
      ...
      await axios
        .put(
          "/api/conferences/" + state.conferenceForm.id,
    ...
    async saveConference({ commit, state }) {
      commit("setConferenceFormLoading", true);
      await axios
        .post("/api/conferences", state.conferenceForm)
        .then(response => {
    ...
    async deleteConference({ commit }, payload) {
      commit("setConferenceFormLoading", true);
      await axios
        .delete("/api/conferences/" + payload)
        .then(() => {
